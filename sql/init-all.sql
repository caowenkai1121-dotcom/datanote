...
-- ===== AI Agent 天工司辰 表(见 sql/70_ai_agent.sql) =====
...
CREATE TABLE IF NOT EXISTS dn_ai_session (
    ...
    autonomous      TINYINT      NOT NULL DEFAULT 0 COMMENT '无人值守自主执行(1=后台驱动器持续推进至计划完成/预算耗尽)',
    auto_max_steps  INT          NOT NULL DEFAULT 0 COMMENT '自主任务全程生产步硬上限(0=不限, 防失控)',
    autonomous_until DATETIME    DEFAULT NULL COMMENT '自主任务墙钟截止(超时自动收尾)',
    last_heartbeat  DATETIME     DEFAULT NULL COMMENT '自主驱动心跳(检测卡死/防多实例重复领取)',
    auto_idle_count INT          NOT NULL DEFAULT 0 COMMENT '自主连续无进展周期数(熔断用)',
