...
public class PromptBuilder {
    ...
    private static final String PROTOCOL =
            ...
            "5. 工具返回 error 按类处理(勿在同一错误上反复空转): ①临时错(超时/连接/网关)→可重试 1-2 次; "
            + "②业务错(权限不足/表或字段不存在/无数据)→【勿重试】, 据错误提示改方案、换正确库表字段或 ask_user 澄清; "
            + "③参数/逻辑错→停止重复, 修正参数或换工具; 连续多次失败就如实说明并给力所能及的答复。\n" +
            ...
            + "   · 【严禁谎称预览】只有【本轮确实调用了 create_artifact 工具】才可说『点右侧预览』; 若没调用就别提右侧预览(否则用户找不到面板)——这种情况把结论/报告直接写进答复正文。\n" +
            ...
            + "(如: 看该表下游影响 / 给它建质量规则 / 出一份分析报告 / 接着建 DWS 汇总), 让用户知道下一步能做什么; 简短即可, 不强加。\n" +
