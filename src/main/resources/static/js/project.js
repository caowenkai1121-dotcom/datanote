...
function projRenderList(list) {
  ...
  if (!rows.length) {
    if (_projList.length) {
      ...
      box.innerHTML = '<div class="gov-empty"><svg viewBox="0 0 24 24" width="44" height="44" fill="none" stroke="currentColor" stroke-width="1.4"><path d="M3 7a2 2 0 012-2h4l2 2h8a2 2 0 012 2v8a2 2 0 01-2 2H5a2 2 0 01-2-2z"/></svg><div class="et">暂无项目</div><div style="font-size:12px;color:var(--text-faint);margin-bottom:4px;">创建第一个项目, 把资产 / 任务 / 成员组织起来</div><button class="btn btn-sm btn-primary" data-perm="project:manage" onclick="if(window.projOpenCreate)projOpenCreate()">＋ 新建项目</button></div>';
    }
    return;
  }
