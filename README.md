...
| 模块 | 功能 |
|------|------|
| **数据同步** | MySQL → Doris 全量/增量同步（DataX 引擎），字段映射，一键建表 |
| **SQL 开发** | Monaco Editor 在线 IDE，Doris SQL 执行，结果排序/筛选/导出 CSV |
...
| 数据库 | MySQL 8.0（元数据） + Apache Doris（数仓） |
| 数据同步 | DataX |
| AI | Claude API（可选，不配置不影响核心功能） |

## 快速开始

分两步部署：DataX（可选）→ DataNote。Doris 连接请通过 `DORIS_HOST` / `DORIS_QUERY_PORT` / `DORIS_PASSWORD` 或「系统管理 → 数据源管理」显式配置。

```bash
git clone https://github.com/datanote1018/datanote.git
cd datanote

# 第一步（可选）：安装 DataX（需要数据同步功能时开启）
...
# → 系统管理 → 数据源管理 → 配置 Doris 连接 → 完成
```

**环境要求**：Java 8+；需要数据同步时额外准备 Docker 或本地 DataX。
...
**Doris / DataX / AI** 在页面上配置：系统管理 → 数据源管理 / 环境配置 / AI 配置。
...
├── setup-datax.sh     # 可选：启动 DataX 容器
├── setup-datanote.sh  # 安装 DataNote
