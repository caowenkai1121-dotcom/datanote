...
### 批 G(R13)— datamodel_generate 工具(模块能力补全)
- **R13 datamodel_generate**: 补 agent 此前只能存/送审/审批模型、无法触发层间生成的缺口。薄适配 DataModelService: physical(逻辑→物理) / logical(业务→逻辑) / reverse(物理表逆向建模)。MEDIUM 写, datamodel:edit。至此 agent 可自动化数据建模的逆向与层间生成。

### 批 H(R14)— datamodel_inspect 只读工具
- **R14 datamodel_inspect**: 只读, 补 agent 对模型的"看": validate(规范校验送审前自检)/ddl(物理模型DDL预览不执行)/versions(版本列表)/standard_impact(标准被哪些模型属性引用)。datamodel:view。与 datamodel_generate/save/submit/approve 构成 agent 完整建模闭环(看-生成-存-送审-审批)。

### 批 I(R15)— run_quality_rule 工具
- **R15 run_quality_rule**: 补 agent "能建质量规则却不能跑"的缺口(薄适配 QualityService.executeRule)。按 ruleId 执行校验, 返回 总数/通过/失败/通过率/错误样例(真值, 据此判断质量勿臆造)。governance:edit。至此 agent: create_quality_rule→run_quality_rule→quality_score 质量闭环。

### 批 J(R16)— calc_metric 工具
- **R16 calc_metric**: 补 agent "能建指标却不能算值"缺口(薄适配 MetricValueService.calc)。按 metricId 执行指标SQL算数值, 返回 指标值/业务日期/状态(真值)。metrics:view。agent 指标闭环: create_metric→calc_metric→metric_detail。
