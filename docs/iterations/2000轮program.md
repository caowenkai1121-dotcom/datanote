...
## R277 [UI重构·第148轮] 数据模型列表 搜索 + 计数
- datamodel.js: 模型列表加"共 N 个模型"计数 + 即时搜索框(编码/名称/类型/数仓层/主题域; 原地隐藏不匹配行不重渲染; 全隐藏给"无匹配模型"反馈)。承质量/dbsync 即时搜索范式。真机验证 17 模型→搜索筛 7→无匹配提示→清空复原。?v=u218。

## R278 [UI重构·第149轮] 数据模型列表 CSV 导出
- datamodel.js: 模型列表加"导出CSV"(编码/名称/类型/数仓层/主题域/状态/版本/实体数; BOM 防乱码; 引号转义)。CSV导出体系再覆盖数据模型。真机验证 触发下载"数据模型列表.csv"。?v=u219。
...
## R281 [UI重构·第152轮] 数据模型详情 实体/属性/关系计数
- datamodel.js: 模型详情抽屉「实体」标题后加"N 个"、每个实体卡片名后加"N 属性"、「关系」标题后加"N 条"。大模型一眼看清规模, 不必逐项数。真机验证 打开模型详情 三处计数均渲染。?v=u222。

## R282 [UI重构·第153轮] 数据地图表清单 计数
- workspace.html: renderDmTable 工具栏加"共 N 张表"计数(与导出CSV同行, 计数左/导出右)。承列表计数范式, 数据地图表浏览一眼知规模。真机验证 "共 N 张表"渲染。?v=u223。

## R283 [UI重构·第154轮] 表字段明细 搜索 + 计数
- workspace.html: dmRenderFields(数据地图打开表→字段明细)加"共 N 字段"计数 + 即时搜索(字段名/中文名/类型; 原地隐藏不匹配行; 全隐藏给"无匹配字段")。宽表几十列时快速定位字段。承即时搜索范式。真机验证 6 字段→搜索筛 2→无匹配→清空复原。?v=u224。
...
## R308 [UI重构·第179轮] 文件树搜索 × 清除按钮
- workspace.html: 文件树搜索框加可见 × 清除按钮(有内容才现, 点击清空+重置筛选+回焦; role=button 键盘可达走全局Enter委托)。原仅 Esc 清除不易发现, 承 DN.table 搜索清除范式。真机验证 初始隐藏→输入显现→点击清空值并隐藏。?v=u249。

## R309 [UI重构·第180轮] 指标搜索 × 清除按钮
- workspace.html: 指标列表搜索框加 × 清除(有内容才现, 点击清空+重载+回焦; oninput 即时切显; metricSearchClear)。承 R308 文件树清除范式。真机验证 隐藏→输入显现→点击清空并隐藏。?v=u250。
...
## R319 [UI重构·第190轮] 资产目录 整行点击进详情
- gov-assets.js: 资产清单主表(assetTbl)接 onRow→openAssetDetail(与树视图行点击/详情按钮同效)。整行点击进资产详情(字段/画像/血缘)。承 DN.table onRow 范式(MDM/标准)。截图复核治理资产目录视觉现代(KPI磁贴/DN.table搜索×/排序↕/DORIS徽标)。真机验证 20 资产行可点, 整行点击打开详情抽屉。?v=u260。

## R320 [UI重构·第191轮] 用户管理搜索 × 清除按钮
- workspace.html: 用户管理搜索框加 × 清除(umSearchClear, 有内容才现+回焦)。即时搜索清除×现覆盖 文件树/指标/质量/数据集成/用户 + DN.table。截图复核系统管理用户表视觉整洁(tab/计数/排序↕/状态pill/导出)。真机验证 隐藏→输入显现→点击清空隐藏。?v=u261。

## R321 [UI重构·第192轮] 数据模型搜索 × 清除按钮(收口全站即时搜索清除)
- datamodel.js: 模型列表搜索框加 × 清除(dmModelSearchClear)。至此全部即时过滤搜索框均有可见×清除: 文件树/指标/质量/数据集成/用户/数据模型 + DN.table工厂。截图复核数据地图页(hero搜索/采集状态/最近表)视觉现代。真机验证 隐藏→输入显现→点击清空隐藏。?v=u262。
...
## R330 [UI重构·第201轮] AI助手 欢迎页示例改可点建议条
- ai-agent.js: 欢迎页示例提示由纯文本改为可点击按钮(3条: 看下治理总览.../建风控项目/某表质量为什么下降), 点击填入输入框并聚焦(现代AI对话建议条范式, 降低上手门槛)。截图复核AI面板视觉现代(数据中心上传/模型选择/经验定时审批/textarea输入)。真机验证 3 chips渲染, 点击首条→AI输入框填入"看下治理总览，再查..."。?v=u271。
...
## R379 [UI重构·第250轮] SQL片段库搜索无匹配反馈
- workspace.html: snippetFilter 搜索过滤后若全部隐藏, 注入 #snippetNoMatch "未找到匹配「x」的片段"(重匹配自动移除/更新文案)。承全站无匹配空态体系(指标/质量/数据集成/用户/项目成员)。真机片段库注入测试 无匹配显示/恢复移除 全对。?v=u321。

## R380 [UI重构·第251轮] SQL片段库搜索清除×
- workspace.html: 片段库搜索框加清除"×"(有输入才显, 全内联 onclick 清空+隐×+重筛+聚焦), 配 R379 无匹配反馈成完整片段搜索体验。即时搜索×体系再+1(文件树/指标/质量/数据集成/用户/模型/数据建模/项目成员/片段库)。真机片段库验证 初始隐/输入显/清除隐+清空+聚焦。?v=u322。

## R381 [UI重构·第252轮] 调度任务搜索清除×
- workspace.html: 数据运维调度任务名搜索框(schedFilterName)加清除"×"(有输入才显, 内联清空+隐×+重筛+聚焦)。调度本已有无匹配/无数据双空态(R240)+命中高亮, 此补清除×。真机验证 初始隐/输入显/清除隐+清空(focus因测试时调度子tab未激活input隐藏未生效, 同范式确认非bug)。?v=u323。
...
## R408 [UI重构·第279轮] AI用户气泡"重问"(填回输入框可编辑重发)
- ai-agent.js: userBubble 含"↻ 重问"链接(>6字才显), 点击把该问题填回输入框+自增高+聚焦, 便于微调后重新发送。配 R394终答复制/R395代码块复制成 AI 消息交互闭环。真机发问→用户气泡现重问链接, 点击回填原问题+聚焦。?v=u350。

## R409 [UI重构·第280轮] 相对时间显示(用户最后登录, GitHub式)
- workspace.html: 新增全局 dnRelTime(刚刚/N分钟前/N小时前/N天前/N个月前/N年前; 未来/空→''); 用户管理"最后登录"改显相对时间, 绝对时间挂 title 悬停兜底(排序仍按ISO不受影响)。真机 dnRelTime 单测全绿 + 用户表显示"8小时前"title="2026-06-16 14:56"。dnRelTime 可后续复用到审计/活动等时间列。?v=u351。
